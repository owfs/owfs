...
/* DS2430A memory */
static int FS_r_application(BYTE *buf, const size_t size, const off_t offset , const struct parsedname * pn) {
    if ( OW_r_app( buf, size, offset, pn ) ) return -EINVAL ;
    return size ;
}
...
static int FS_w_application(const BYTE *buf, const size_t size, const off_t offset , const struct parsedname * pn) {
    if ( OW_w_app(buf,size, (size_t) offset,pn) ) return -EINVAL ;
    return 0 ;
}
...
static int OW_w_app( const BYTE * data , const size_t size, const off_t offset , const struct parsedname * pn ) {
    ...
    /* load scratch pad if incomplete write */
    if ( (size!=8) && BUS_transaction( tread, pn ) ) return 1 ;
    /* write data to scratchpad */
    if ( BUS_transaction( twrite, pn ) ) return 1 ;
    /* read back the scratchpad */
    if ( BUS_transaction( tver, pn ) ) return 1 ;
