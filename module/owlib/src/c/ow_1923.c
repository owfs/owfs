...
struct filetype DS1923[] = {
  ...
  {"mission/samplingtemp", 1, NULL, ft_yesno, fc_volatile, {y: FS_bitread}, {v: NULL}, {v:&BitReads
	    [3]},},
  {"mission/samplinghumidity", 1, NULL, ft_yesno, fc_volatile, {y: FS_bitread}, {v: NULL}, {v:&BitReads
	    [4]},},
#if 0
...
{
    BYTE data[3] =
	{ u[0] & 0xFF, (u[0] >> 8) & 0xFF, (u[0] >> 16) & 0xFF };
    if (OW_MIP(pn))
	return -EBUSY;
    if (OW_w_mem(data, 3, 0x0216, pn))
	return -EINVAL;
    return 0;
}

/* Just a test-function */
static int FS_enable_osc(BYTE * buf, const size_t size, const off_t offset,
			 const struct parsedname *pn)
{
...
static int FS_clearmem(BYTE * buf, const size_t size, const off_t offset,
		       const struct parsedname *pn)
{
    (void) offset;		// to quash compiler noise
    memset(buf, 0, 1);
...
static int FS_r_temperature(_FLOAT * T, const struct parsedname *pn)
{
    ...
    if (OW_r_temperature(T, delay, pn))
	return -EINVAL;
    return 0;
}

static int FS_r_humid(_FLOAT * H, const struct parsedname *pn)
{
    UINT delay = 666;
    int ret;
    if ((ret = OW_MIP(pn))) {
