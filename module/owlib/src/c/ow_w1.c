...
static int sendback(int seq, const BYTE * data, BYTE * resp, const size_t size, const struct parsedname *pn)
{
	int ret = -EINVAL ;
...
{
	return sendback( w1_send_selecttouch(data,size,pn), data, resp, size, pn) ;
}
...
{
	return sendback( w1_send_touch(data,size,pn), data, resp, pn) ;
}
