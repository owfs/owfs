...
static int OW_r_ulong(uint64_t * L, const size_t size, const off_t offset,
		      const struct parsedname *pn);
static int OW_w_ulong(const uint64_t * L, const size_t size,
		      const off_t offset, const struct parsedname *pn);
...
//    if ( OW_w_mem( buf, size, (size_t) offset, pn) ) return -EFAULT ;
    if (OW_write_paged(buf, size, (size_t) offset, pn, 32, OW_w_mem))
	return -EFAULT;
    return 0;
}
...
{
    BYTE c;
    if (OW_r_mem_simple(&c, 1, 0x200, pn))
	return -EINVAL;
    u[0] = Avals[c & 0x07];
    return 0;
}
...
static int FS_w_flag(const int *y, const struct parsedname *pn)
{
    ...
    if (OW_r_mem_simple(&cr, 1, 0x0201, pn))
	return -EINVAL;
    if (y[0]) {
	if (cr & fl)
	    return 0;
		      ...
		      const struct parsedname *pn)
{
    ...
    if (size > 5)
	return -ERANGE;
    if (OW_r_mem_simple(data, size, offset, pn))
	return -EINVAL;
    L[0] = ((uint64_t) data[0])
	+ (((uint64_t) data[1]) << 8)
	+ (((uint64_t) data[2]) << 16)
