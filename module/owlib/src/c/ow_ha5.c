...
static int HA5_sendback_part(const BYTE * data, BYTE * resp, const size_t size, const struct parsedname *pn) ;
...
int HA5_detect(struct connection_in *in)
{
	...
	if (HA5_reset(&pn) == BUS_RESET_OK ) {
		in->Adapter = adapter_HA5 ;
		in->adapter_name = "HA5/S";
		in->AnyDevices = 1 ;
		return 0;
	} else if (HA5_reset(&pn) == BUS_RESET_OK ) {
		in->Adapter = adapter_HA5 ;
		in->adapter_name = "HA5/S";
		in->AnyDevices = 1 ;
		return 0;
	}
	LEVEL_DEFAULT("HA5 detection error\n");
	return -ENODEV;
}
...
static int HA5_reset(const struct parsedname *pn)
{
	...
	if (resp[0]!=0x0D) {
		LEVEL_DEBUG("Error HA5 reset bad <cr>\n");
		return -EIO;
	}
	return BUS_RESET_OK;
}
...
static int HA5_select( const struct parsedname * pn )
{
	...
	if ( memcmp( pn->sn, pn->selected_connection->connin.ha5.sn, 8 ) ) {
		...
		if ( COM_write((BYTE*)"M",1,pn) ) {
			LEVEL_DEBUG("Error with sending HA5 M-atch\n") ;
			return HA5_resync(pn) ;
		}
	}
	...
	// Set as current "Address" for adapter
	memcpy( pn->selected_connection->connin.ha5.sn, pn->sn, 8) ;
...
static int HA5_sendback_data(const BYTE * data, BYTE * resp, const size_t size, const struct parsedname *pn)
{
	...
	for ( left=size ; left>0 ; left -= 32 ) {
		...
		if ( HA5_sendback_part( &data[pass_start], &resp[pass_start], pass_size, pn ) ) {
			return -EIO ;
		}
	}
	return 0;
