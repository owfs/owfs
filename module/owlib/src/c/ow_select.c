...
int BUS_select(const struct parsedname * pn) {
    if ( pn->pathlength > 0 && (pn->in->iroutines.flags & ADAP_FLAG_2409path) ) {
...
static int Turnoff( int depth, const struct parsedname * pn ) {
    ...
    if ( depth && (ret=BUS_select_subbranch(&(pn->bp[depth-1]),pn)) ) return ret ;
