...
static int OW_w_wiper(const unsigned int val, const struct parsedname * pn) {
    ...
    BUSLOCK
        ret = BUS_select(pn) || BUS_send_data(cmd,2) || BUS_send_data(&ninesix,1) || BUS_readin_data(&resp,1) || (resp!=val) || BUS_sendback_data(&ninesix,&resp,1) || resp ;
    BUSUNLOCK
    return ret ;
}
...
static int OW_w_cp(const int val, const struct parsedname * pn) {
    ...
    BUSLOCK
        ret = BUS_select(pn) || BUS_send_data(cmd,2) || BUS_send_data(&ninesix,1) || BUS_readin_data(&resp,1) || (resp!=cmd[1]) || BUS_sendback_data(&ninesix,&resp,1) || resp ;
    BUSUNLOCK
    return ret ;
}
