...
GOOD_OR_BAD Netlink_Parse_Get( struct netlink_parse * nlp )
{
	...
	int recv_len = recv( Inbound_Control.w1_monitor->head->file_descriptor, &peek_nlm, W1_NLM_LENGTH, MSG_PEEK );
	...
	// read whole packet
	recv_len = recv( Inbound_Control.w1_monitor->head->file_descriptor, &(nlp->nlm[0]), peek_nlm.nlmsg_len, 0 );
	if (recv_len == -1) {
