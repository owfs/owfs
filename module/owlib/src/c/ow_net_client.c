...
GOOD_OR_BAD ClientAddr(char *sname, char * default_port, struct connection_in *in)
{
	...
	case 0: // Complete default address
		in->master.tcp.host = NULL;
		in->master.tcp.service = owstrdup(default_port);
		break ;
	case 1: // single entry -- usually port unless a dotted quad
		...
		case address_dottedquad:
			...
			in->master.tcp.host = owstrdup(ap.first.alpha);
			in->master.tcp.service = owstrdup(default_port);
			break ;
		default:
			...
			if ( strcasecmp( default_port, DEFAULT_SERVER_PORT ) == 0 ) {
				in->master.tcp.host = NULL;
				in->master.tcp.service = owstrdup(ap.first.alpha);
			} else {
				in->master.tcp.host = owstrdup(ap.first.alpha);
				in->master.tcp.service = owstrdup(default_port);
			}
	...
	default: // address:port format -- unambiguous
		in->master.tcp.host = ( ap.first.type == address_none ) ? NULL : owstrdup(ap.first.alpha) ;
		in->master.tcp.service = ( ap.second.type == address_none ) ? owstrdup(default_port) : owstrdup(ap.second.alpha) ;
		break ;
	}
	Free_Address( &ap ) ;
	...
	if(in->master.tcp.host == NULL) {
		/* getaddrinfo doesn't work with host=NULL for cygwin */
		in->master.tcp.host = owstrdup("127.0.0.1");
	}
#else
	hint.ai_family = AF_UNSPEC;
#endif

	LEVEL_DEBUG("IP address=[%s] port=[%s]", SAFESTRING(in->master.tcp.host), in->master.tcp.service);
	ret = getaddrinfo(in->master.tcp.host, in->master.tcp.service, &hint, &in->master.tcp.ai) ;
	if ( ret != 0 ) {
...
{
	SAFEFREE(in->master.tcp.host) ;
	SAFEFREE(in->master.tcp.service) ;
	if (in->master.tcp.ai) {
		freeaddrinfo(in->master.tcp.ai);
		in->master.tcp.ai = NULL;
	}
	in->master.tcp.ai_ok = NULL;
}
...
FILE_DESCRIPTOR_OR_ERROR ClientConnect(struct connection_in *in)
{
	...
	ai = in->master.tcp.ai;		// loop from first address info since it failed.
	do {
		file_descriptor = socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol);
		if ( FILE_DESCRIPTOR_VALID(file_descriptor) ) {
			if (connect(file_descriptor, ai->ai_addr, ai->ai_addrlen) == 0) {
				in->master.tcp.ai_ok = ai;
				return file_descriptor;
			}
			close(file_descriptor);
		}
