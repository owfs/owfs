...
class DS2409( unittest.TestCase ):
    ...
    def testAttributes( self ):
        for name in sensors:
            ...
            self.failUnlessEqual( hasattr( sensor, 'aux' ),         True )
            self.failUnlessEqual( hasattr( sensor, 'branch.0' ),    True )
            self.failUnlessEqual( hasattr( sensor, 'branch.1' ),    True )
            self.failUnlessEqual( hasattr( sensor, 'branch.ALL' ),  True )
            self.failUnlessEqual( hasattr( sensor, 'branch.BYTE' ), True )
            self.failUnlessEqual( hasattr( sensor, 'control' ),     True )
            #self.failUnlessEqual( hasattr( sensor, 'discharge' ),   True )
            self.failUnlessEqual( hasattr( sensor, 'event.0' ),     True )
            self.failUnlessEqual( hasattr( sensor, 'event.1' ),     True )
            self.failUnlessEqual( hasattr( sensor, 'event.ALL' ),   True )
            self.failUnlessEqual( hasattr( sensor, 'event.BYTE' ),  True )
            self.failUnlessEqual( hasattr( sensor, 'main' ),        True )
