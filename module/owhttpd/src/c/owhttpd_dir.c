...
#if OW_MT
pthread_mutex_t ShowDirmutex = PTHREAD_MUTEX_INITIALIZER ;
#endif /* OW_MT */

const struct parsedname * ShowDirpn;
FILE * ShowDirout = NULL;
struct parsedname ShowDirpncopy;

/* Callback function to FS_dir */
void ShowDirdirectory( const struct parsedname * const pn2 ) {
    /* uncached tag */
    /* device name */
    ...
    *buffer = '\000';
    ...
    if (ShowDirpn->state & pn_text) {
        fprintf( ShowDirout, "%s %s \"%s\"\r\n", loc, nam, typ ) ;
    } else {
        fprintf( ShowDirout, "<TR><TD><A HREF='%s/%s'><CODE><B><BIG>%s</BIG></B></CODE></A></TD><TD>%s</TD><TD>%s</TD></TR>", (strcmp(ShowDirpncopy.path,"/")?ShowDirpncopy.path:""), loc, loc, nam, typ ) ;
...
void ShowDir( FILE * out, const struct parsedname * const pn ) {
...
#if OW_MT
    pthread_mutex_lock(&ShowDirmutex) ;
#endif /* OW_MT */
    ShowDirout = out;
    ShowDirpn = pn;
    memcpy(&ShowDirpncopy, pn, sizeof(struct parsedname));
    ...
    if(pn->state & pn_text) {
      FS_dir( ShowDirdirectory, &ShowDirpncopy ) ;
      return;
    }
    ...
    HTTPfoot( out ) ;
#if OW_MT
    pthread_mutex_unlock(&ShowDirmutex) ;
#endif /* OW_MT */
}
